...
class SyncedDatasets(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return tuple(it.chain(*(i if isinstance(i, tuple) else (i,) for i in items)))
...
class LambdaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        temp_ret[self.at_index] = functools.reduce(
            lambda x, f: f(x), self.func, temp_ret[self.at_index]
        )
